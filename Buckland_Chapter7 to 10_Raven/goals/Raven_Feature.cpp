...
double GetMaxRoundsBotCanCarryForWeapon(int WeaponType)
{
  ...
  case type_grenade:
	  ...
	  return script->GetDouble("Grenade_MaxRoundsCarried");
...
double Raven_Feature::TotalWeaponStrength(Raven_Bot* pBot)
{
  ...
  const double MaxRoundsForGrenade = GetMaxRoundsBotCanCarryForWeapon(type_grenade);
  const double TotalRoundsCarryable = MaxRoundsForShotgun + MaxRoundsForRailgun + MaxRoundsForRocketLauncher + MaxRoundsForGrenade;
  ...
  return Tweaker + (1-Tweaker)*(NumSlugs + NumCartridges + NumRockets + NumGrenades)/(MaxRoundsForShotgun + MaxRoundsForRailgun + MaxRoundsForRocketLauncher + MaxRoundsForGrenade);
}
